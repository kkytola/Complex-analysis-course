...
\begin{lemma}[Discontinuity of the principal argument]
  \label{lem:discontinuity_of_principal_arg}
  \uses{def:argument, def:continuous, lem:metric_on_C}
  % \lean{}
  % \leanok
...
\begin{definition}[Complex exponential function]
  ...
  \uses{def:complex_numbers, thm:complex_number_field}
  % \lean{}
  % \leanok
...
\begin{lemma}[Properties of the complex exponential]
  \label{lem:complex_exp_properties}
  \uses{def:complex_exp}
  % \lean{}
  % \leanok
  For any $z,w \in \bC$ we have
  \begin{align*}
...
\begin{definition}[Principal complex logarithm]
  ...
  % \lean{}
  % \leanok
  The \term{principal logarithm} is the function
  \begin{align*}
...
\begin{definition}[Branches of complex logarithm]
  \label{def:branch_of_log}
  \uses{def:complex_exp, def:open_set, def:continuous, def:principal_complex_log}
  % \lean{}
  % \leanok
  A \term{branch of the logarithm} is a continuous function
...
\begin{definition}[Principal complex power function]
  \label{def:principal_complex_power}
  \uses{def:principal_complex_log}
  % \lean{}
  % \leanok
  Let $\alpha \in \bC$. The \term{principal (complex) $\alpha$th
  power function} is the function $\bC \setminus \set{0} \to \bC$
  given by
  \begin{align*}
...
Given a branch $\ell \colon U \to \bC$ of logarithm on an
...
\begin{definition}[Principal $n$th root function]
  ...
  \uses{def:principal_complex_log, def:principal_complex_power, lem:roots_of_unity}
  % \lean{}
  % \leanok
  Let $n \in \bN$. The \term{principal (complex) $n$th
  root} of $z \in \bC \setminus \set{0}$ is
  \begin{align*}
