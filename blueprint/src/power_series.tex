...
\begin{definition}[Complex series \Palka{Sec.~VII.2.1}]
  ...
  Define their \term{partial sums} as
  \begin{align*}
    S_N \; = \; \sum_{n=1}^N z_n \; = \; z_1 + z_2 + \cdots + z_N
    \qquad \text{ for } N \in \bN \,.
  \end{align*}
...
\begin{definition}[Series of functions \Palka{Sec.~VII.2.2}]
  ...
  Define their \term{partial sums} as the functions given by
  \begin{align*}
    F_N(z) \; = \; \sum_{n=1}^N f_n(z) \; = \; f_1(z) + f_2(z) + \cdots + f_N(z)
  ...
  Notation $\sum_{n=1}^\infty f_n$ is then used to denote the limit function.
  %\begin{align*}
...
\begin{lemma}[Hadamard's formula for the radius of convergence \Palka{Thm~VII.3.3}]
  ...
  \uses{def:power_series, def:limsup}
  %\lean{}
  % \leanok
  Let $z_0 \in \bC$ be a point in the complex plane
  and let $a_0,a_1,a_2\ldots \in \bC$ be coefficients.
