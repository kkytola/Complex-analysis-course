...
\begin{definition}[Compactness]
  ...
  % \leanok
  Let $X$ be a metric space.
  A subset $K \subset X$ is \term{compact} if every sequence
...
\begin{theorem}[Cantor's intersection theorem \Palka{Thm~II.4.5}]
...
\begin{definition}[Path homotopy for closed paths]
  ...
  %with the same endpoints $\gamma_0(a) = \gamma_1(a)$
  %and $\gamma_0(b) = \gamma_1(b)$.
  If there exists a
  continuous function (called a \term{homotopy})
  \begin{align*}
  ...
  \begin{align*}
    ...
    %& \Gamma(s,a) = \gamma_0(a) \quad \text{ and } \Gamma(s,b) = \gamma_0(b)
...
\begin{definition}[Contractible path]
  ...
  \term{contractible} if it is homotopic to a constant path.
\end{definition}

\begin{definition}[Simple connectedness]
  \label{def:simply_connected}
  \uses{def:contractible}
  % \lean{}
  % \leanok
  A metric space is said to be \term{simply connected}
